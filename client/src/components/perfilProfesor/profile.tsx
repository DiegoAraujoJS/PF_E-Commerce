...
function Profile(email, { user }) {
        ...
        <div>
          <div className={style.container}>
              ...
              <div className={style.profileContainer}>
                <div className={style.icon}>
                  ...
                  <p className="small mb-4">
                    {" "}
                    <i className="fas fa-map-marker-alt mr-2"></i>Editar tu
                    perfil
                  </p>
                </div>
                ...
                {prof.howMany >= 0 ? (
                  <RatingView ratingValue={prof.score} /* RatingView Props */ />
                ) : (
                  "Este profesor no tiene suficientes reviews"
                )}
            ...
            <section className={style.sectionTwo}>
              <h4
                className={
                  style.h4Prof + " d-flex justify-content-center pt-4 m-0"
                }
                style={{ color: "#0d6efd" }}
              >
                Aca podrás ver sus horarios disponibles:
              </h4>
              <br />
              <div className="d-flex justify-content-center mb-3">
                <Link to="/calendarAdd">
                  <button
                    className={style.agregaDisponibilidad}
                    style={{ width: "400px" }}
                  >
                    Agrega un nueva disponibilidad horaria
                  </button>
                </Link>
              </div>
    ...
    try {
      if (propEmail) {
        const response = await axios.get(
          `http://localhost:3001/api/usuarios/${propEmail.email}`
        );
  ...
  const fetchProfs = async () => {
    try {
      if (propEmail) {
        const response = await axios.get(
          `http://localhost:3001/api/profesores/${propEmail.email}`
        );
